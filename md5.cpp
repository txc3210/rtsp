...
void GetMD5(char *buf,std::string& strMD5)
{
	unsigned char decrypt[16];
	MD5_CTX md5;
	MD5Init(&md5);
	//can not use static_cast form char* to unsigned char*
	MD5Update(&md5,reinterpret_cast<unsigned char*>(buf),static_cast<unsigned int>(strlen(buf)));
	MD5Final(&md5,decrypt);
	char str[33];
	for(int i=0;i<16;i++)
	{
		snprintf(str+i*2,sizeof(str)-i*2,"%02x",decrypt[i]);
	}
	strMD5.clear();
	strMD5.append(str);
}
